...
//输出
imagettftext($im, 14, 0, 10, 40, $red, $font, 'Your Location '.$country.','.$region.','.$city.'');
imagettftext($im, 14, 0, 10, 72, $red, $font, 'Today is '.date('Y/n/j')."  ".$weekarray[date("w")]);//当前时间添加到图片
imagettftext($im, 14, 0, 10, 104, $red, $font,'Your IP is: '.$ip.'');//ip
imagettftext($im, 14, 0, 10, 140, $red, $font,'Your OS is '.$os.'');
imagettftext($im, 14, 0, 10, 175, $red, $font,'You Broswer is '.$bro.'');
imagettftext($im, 14, 0, 10, 200, $black, $font,$get); 

//imagettftext($im, 12, 0, 10, 200, $black, $font,'Been Seen '.$counter.' Times'); 
ImageGif($im);
ImageDestroy($im);
?>
<?php
